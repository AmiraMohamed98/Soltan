...
public class AddSoud extends AppCompatActivity {
    ...
    Bundle parameters;
    String title,subTitle,sub;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_add_soud);
        parameters= getIntent().getExtras();
       // title=parameters.getString("title");
        //subTitle=parameters.getString("subTitle");
...
//
    ...
    private void uploadPdf(Uri sUri) {
        ...
        progressDialog.show();
        if (sub.equals("not")) {
            storageRef = storageRef.child("audio").child(title).child(name);
            storageRef.putFile(sUri)
                    .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                        ...
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            ...
                            db.collection("Audio").document(title)
                                    .collection("0").document(edit_pdf_name.getText().toString())
                                    .set(map)
                                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                                        @Override
                                        public void onSuccess(Void unused) {
                                            Toast.makeText(AddSoud.this, "تم إضافه الكتاب", Toast.LENGTH_SHORT).show();
                                            edit_pdf_name.setText("");
                                            progressDialog.dismiss();
                                        }
                                    }).addOnFailureListener(new OnFailureListener() {
                                        @Override
                                        public void onFailure(@androidx.annotation.NonNull Exception e) {
                                            Toast.makeText(AddSoud.this, "لم يتم إضافه الكتاب", Toast.LENGTH_SHORT).show();
                                            progressDialog.dismiss();
                                        }
                                    });
                        }
            ...
            storageRef = storageRef.child("audio").child(title).child(sub).child(name);
            storageRef.putFile(sUri)
                    .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                        @Override
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            ...
                            db.collection("Audio").document(title)
                                    ...
                                    .document(edit_pdf_name.getText().toString()).set(map)
                                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                                        @Override
                                        public void onSuccess(Void unused) {
                                            Toast.makeText(AddSoud.this, "تم إضافه الكتاب", Toast.LENGTH_SHORT).show();
